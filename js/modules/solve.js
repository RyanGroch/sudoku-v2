export default async function solve(args) {
  ...
  const cells = args.cellClassName
    ? document.querySelectorAll(`.${args.cellClassName}`)
    : null;
  const givenSolution = args.solution || null;

  const numSolutions = await next(
    board,
    rowStart,
    colStart,
    digits,
    solutions,
    args.getFirstSolution,
    cells,
    givenSolution
  );
...
async function next(
  ...
  if (board[row - 1][col - 1]) {
    updatedSolutionsNum = await next(
      board,
      nextRow,
      nextCol,
      digits,
    ...
    for (const digit of shuffledDigits) {
      ...
      if (cells)
        await updateCell(cells, row - 1, col - 1, digit, givenSolution);

      const cell3x3 = getCell3x3(board, row, col);

      if (
        rowConflict(board, row) ||
        colConflict(board, col) ||
        cellConflict(cell3x3)
      ) {
        board[row - 1][col - 1] = 0;
        continue;
      }
