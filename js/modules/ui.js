...
export default class UserInterface {
  ...
  eraseCell(e) {
    ...
    if (e) {
      ...
      const eraseValue = containsNotes
        ? getCellNotes(this.selectedCell)
  ...
  async handleAsNote(enteredNumber, triggeredByEvent) {
    if (
      this.selectedCell.textContent &&
      !this.selectedCell.classList.contains("notes")
    )
      return;
    ...
    const noteElement = this.selectedCell.querySelector(
      `.note-${enteredNumber}`
    );
  ...
  enterAsAnswer(row, col, enteredNumber, triggeredByEvent) {
    ...
    if (triggeredByEvent) {
      ...
      const action = hasNotes ? "changeNotesToInput" : "replaceValue";
...
function getCellNotes(cell) {
  ...
  for (const note of notes) {
    if (note.textContent) notesValues.push(note.textContent);
  }
